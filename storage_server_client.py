...
def create_empty_file(sock: socket.socket, file_name):
    send_int32(sock, CMD_CREATE_EMPTY_FILE)
    send_str(sock, file_name)
    code = receive_int32(sock)
    if code != CODE_OK:
        print('error with code %d' % code)
...
def send_command_to_storage_server(host: str, cmd: int, args=[]):
    ...
    if cmd == CMD_WRITE_FILE and len(args) == 2:
    ...
    elif cmd == CMD_CREATE_EMPTY_FILE and len(args) == 1:
